...
# --------------------------------------------------------------------------- #
# 3.5 Shell completion (under the prefix, not in $HOME)
#
# typer's own `tessera --install-completion zsh` writes ~/.zfunc/_tessera in
# the user's home. To keep all Tessera state under the prefix we instead
# generate it into ~/.local/share/tessera/zfunc/_tessera via the CLI command.
# --------------------------------------------------------------------------- #
if command -v "${TESSERA_BIN}" >/dev/null 2>&1; then
    "${TESSERA_BIN}" completion install --shell zsh 2>&1 \
        | while IFS= read -r line; do info "${line}"; done \
        || warn "completion install skipped (non-fatal)"
else
    warn "tessera binary not on PATH; skipping completion install"
fi
